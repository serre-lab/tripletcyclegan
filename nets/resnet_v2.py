...
class ResNet50:
    ...
    def __init__(self,
        ...
        if lbls_style is not None:
            self.style_lbls = tf.reshape(lbls_style, [-1, cfg.batch_size])
        else:
            self.style_lbls = tf.placeholder(tf.int32, shape=(batch_size, cfg.num_classes), name='style_lbls')
        ...
        # if is_training:
        if images_ph is not None and cfg.aug_style=='img':
        ...
        with slim.arg_scope(resnet_arg_scope()):
            ...
            _, train_end_points = resnet_v2_50(aug_imgs, cfg.num_classes, is_training=True,
                             ...
                             reuse=reuse, scope=scope)
            _, train_end_features = resnet_v2_50(aug_imgs, 0, is_training=False,
                             global_pool=global_pool, output_stride=output_stride,
                              spatial_squeeze=spatial_squeeze,
        ...
        self.val_end_features , self.val_features_labels,self.train_end_features , self.train_end_labels = val_end_features, tf.argmax(self.gt_lbls, 1),train_end_features, tf.argmax(self.gt_lbls, 1);
